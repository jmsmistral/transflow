...
fn sigkill_at_every_durability_boundary_preserves_valid_heads_and_replay_identity() {
 ...
 let build_state:String=sqlx::query_scalar("SELECT b.state FROM builds b JOIN jobs j ON j.build_id=b.id JOIN attempts a ON a.job_id=j.id WHERE a.id=?").bind(AttemptId::from_bytes([11;16]).to_string()).fetch_one(&mut db).await.unwrap();assert_eq!(build_state,if committed{"SUCCEEDED"}else{"INTERRUPTED"});
